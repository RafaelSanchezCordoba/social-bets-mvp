...
export function MobileBottomNav() {
  ...
  return (
    <nav
      aria-label="Primary"
      className="fixed bottom-[max(0.75rem,env(safe-area-inset-bottom))] left-1/2 z-50 w-[calc(100%-1.5rem)] max-w-2xl -translate-x-1/2 px-0"
    >
      <div className="border-line-strong grid grid-cols-5 rounded-[1.7rem] border bg-[rgba(255,250,243,0.96)] p-2 shadow-[0_18px_40px_rgba(65,45,24,0.14)] backdrop-blur">
        {navItems.map((item) => {
          ...
          return (
            <Link
              key={item.href}
              href={item.href}
              className={`flex min-w-0 flex-col items-center justify-center gap-1 rounded-[1rem] px-2 py-2 text-[10px] font-medium transition ${
